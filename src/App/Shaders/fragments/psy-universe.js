...
//#define TIMES_DETAILED (5.0+sin(ctime*PI*1.0)*.1)
...
#define EPS (2.0/max(resolution.x, resolution.y))

#define ctime ((saw(float(__LINE__))+.5)*(time/PI+12345.12345))
...
vec2 mobius(vec2 uv)
{
	vec2 a = saw(seedling+vec2(ctime*1.0*PI, ctime*GR/E*1.0*PI))*.25+.25;
	vec2 b = sin(seedling+vec2(ctime*1.0*PI, ctime*GR/E*1.0*PI));
	vec2 c = saw(seedling+vec2(ctime*1.0*PI, ctime*GR/E*1.0*PI))*.25+.25;
	vec2 d = sin(seedling+vec2(ctime*1.0*PI, ctime*GR/E*1.0*PI));
...
float lowAverage()
{
    ...
    float last = length(texture(sound, vec2(0.0)));
    float next;
    for(int i = 1; i < iters; i++)
    {
        next = length(texture(sound, vec2(float(i)/float(iters), 0.0)).r);
...
void main()
{
    ...
    float swirl = clamp(saw(ctime)*3.0-1.0, 0.0, 1.0);
    float scale = 2.0*PI;
    uv *= pow(scale, swirl);
    uv -= scale/2.0*swirl;
	uv.x *= pow(resolution.x/resolution.y, swirl);
    uv += scale*vec2(cos(time*.234), sin(time*.345))*1.0*swirl;
    ...
    for(int i = 0; i < max_i; i++)
    {
        ...
        //if(mod(time, float(max_i))-float(i) < 0.0) break;
    }
    
    
    float s = simplex3d(vec3(uv*resolution.xy, ctime));
    float sound = texture(sound, saw(uv*PI+gl_FragCoord.xy/resolution.xy*PI+ctime*PI)).x;
    //gl_FragColor = vec4(vec3(sound), 1.0);return;
    ...
    gl_FragColor = vec4(vec3(ifs), 1.0);

    gl_FragColor = vec4(saw((depth)));
    gl_FragColor = vec4(final, 1.0)*smoothstep(0.0, .01, height*sound);
    gl_FragColor.a = 1.0;
    //gl_FragColor = vec4(simplex3d(vec3(gl_FragCoord.xy/resolution.xy, ctime)));
}`
]